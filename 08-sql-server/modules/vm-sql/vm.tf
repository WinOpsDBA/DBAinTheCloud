...
resource "azurerm_virtual_machine_extension" "ps1" {
  count = "${var.server_count}"
  # name  = "${azurerm_virtual_machine.vm1.name}-sql${count.index + 1}-custom-ps1"
  name = "${var.prefix}-sql${count.index + 1}-localPS"
  # virtual_machine_id         = azurerm_virtual_machine.vm1.id
  virtual_machine_id = "${element(azurerm_virtual_machine.vm1.*.id, count.index)}"
  #virtual_machine_name       = "${var.prefix}-sql${count.index + 1}"
  publisher                  = "Microsoft.Compute"
  ...
  protected_settings = <<PROTECTED_SETTINGS
    {
        "commandToExecute":"powershell -ExecutionPolicy Unrestricted -File sql-install.ps1 ${var.storage_name} ${var.adminpassword}"
    }
  PROTECTED_SETTINGS
